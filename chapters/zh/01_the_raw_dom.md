...
## 1.1 从零开始的挑战
 ...
 Po 轻轻敲了敲门，走进了房间。Shifu 正在闭目养神，面前放着一杯热茶。

**🐼**：Shifu，我想要学习 React。我听说它是构建现代 Web 应用的最佳工具，我想知道它是如何工作的。

**🧙‍♂️**：React 确实是一把利剑。但通过它，你想要解决什么问题呢？

**🐼**：解决……构建界面的问题？大家都说用它写代码更清晰，维护更容易。

**🧙‍♂️**：为了理解“清晰”，我们必须先见识“混沌”。如果不经历痛楚，就无法感激良药。在开始学习 React 之前，我们要先回到原点。

**🐼**：回到原点？

**🧙‍♂️**：是的。忘掉所有的框架。没有 React，没有 Vue，也没有 Angular 和 Svelte。请用最原始的 JavaScript，构建一个 **待办事项列表 (Todo List)**。
需求如下：
...
**🐼**：只用原生 JavaScript 吗？这听起来并不复杂。我会尝试一下。

## 1.2 Po 的尝试 (The Imperative Way)

Po 打开编辑器，创建了 `index.html` 和 `app.js`。一段时间后，他展示了自己的成果。
...
**🐼**：Shifu，我完成了。逻辑很直观：获取输入，创建元素，绑定事件，插入 DOM。这就是最纯粹的 JavaScript，不是吗？

## 1.3 痛点分析：命令式编程

**🧙‍♂️**：代码确实可以运行。但是，Po，当你编写这段代码时，你的思维方式是怎样的？

**🐼**：我在思考步骤。首先拿到值，然后造个标签，再把它放进去……

**🧙‍♂️**：正是如此。这就是 **命令式编程** 。你像一个工头，指挥着浏览器这个工人，一步一步地告诉它该做什么。
...
**🐼**：给计算机下达指令，这难道不是编程的本义吗？

**🧙‍♂️**：对于简单的任务，这确实有效。但如果需求变得复杂呢？
假设现在的需求变更：**“当列表为空时，显示'暂无数据'；当有数据时，隐藏这句话。”**

**🐼**：那我可以在添加和删除的时候，加一个检查判断。
...
**🧙‍♂️**：好的，第二个需求变更来了：**“给每个 todo 增加一个复选框，标记完成/未完成。并在列表上方实时显示'已完成 X / 总共 Y 项'的统计。”**

**🐼**：嗯……那我需要给每个 `li` 创建一个 `checkbox`，绑定 `change` 事件来切换样式。统计的话，我需要在 添加、删除、以及每次切换复选框的时候，都去遍历一遍列表里的所有元素来计数……
...
**🧙‍♂️**：你发现了吗？仅仅两个需求变更，你的代码已经变得杂乱了。每当你修改 UI 的一部分（添加、删除、切换状态），你都必须记得去更新 **所有** 与之关联的部分（空状态提示 **和** 统计数字）。一旦遗漏一个 `updateStats()` 调用，界面就会出 Bug。
随着应用变大，这种手动维护的依赖关系会像藤蔓一样缠绕在一起，最终变成难以维护的 **面条代码 (Spaghetti Code)**。

**🐼**：我明白您的意思了。手动管理每一个状态的同步确实很累。

## 1.4 性能隐患：回流与重绘 (Reflow & Repaint)

**🧙‍♂️**：除了维护性的问题，还有一个隐藏的代价——**性能**。
当你执行 `list.appendChild(li)` 时，浏览器并非只是简单的“画上去”。

**🐼**：它还需要做什么？

**🧙‍♂️**：它必须重新计算布局（Reflow），确定每个元素的位置和大小，然后重新绘制（Repaint）。
如果你在一个循环中操作 DOM：
...
for (let i = 0; i < 1000; i++) {
  ...
  list.appendChild(li);
  li.offsetHeight; // 强制浏览器立即计算布局
}
```

这就像是你每搬一块砖，就要求工人重新测量整栋楼的尺寸。虽然现代浏览器会做优化（批量合并），但这种“随心所欲”的修改方式，始终潜藏着性能风险。 之后我会给你一个 demo，你将亲手体验一键插入 5000 个任务后的卡顿。

## 1.5 状态的迷失 (State vs DOM)

**🧙‍♂️**：最后一个，也是最根本的问题。Po，在这个应用中，**数据 (State)** 究竟在哪里？

**🐼**：数据……就是列表里的那些 `li` 元素吧？checkbox 的选中状态也在 DOM 上。

**🧙‍♂️**：没错。你的数据 **寄生在 DOM 之中**。
...
**🧙‍♂️**：在于 **分离**。数据 (State) 应该是唯一的真相来源，而视图 (View) 只是它的投影。当数据改变时，视图应该自动更新以反映最新的状态，而不是由我们手动去一处处修补 DOM。
但在原生 JavaScript 的时代，要做到这一点，我们需要付出巨大的努力。

## 1.6 回头看看

**🧙‍♂️**：现在，你看到了原生 DOM 操作的局限性了吗？

1. **命令式**：繁琐的步骤指令，每个需求变更都导致代码膨胀。
2. **性能**：昂贵的 DOM 操作，缺乏批量更新意识。
3. **耦合**：数据与视图混杂，“真相”散落在 DOM 各处。

**🐼**：是的，Shifu。功能越多，需要手动同步的地方就越多，漏掉任何一处都是 Bug。这样写下去，迟早会被自己的代码淹没。我们该如何改进呢？

**🧙‍♂️**：为了从繁琐的指令中解脱，我们必须改变思考方式。我们不应再像工头一样下达“如何做”的指令，而应该直接描述“要什么”。

**🐼**：描述“要什么”？如果不一步步告诉浏览器怎么做，它怎么知道怎么画呢？

**🧙‍♂️**：我们可以把界面看作一段文本，一段描述结构的字符串。与其精雕细琢每一块砖，不如直接打印整面墙。
...
将以下代码保存为 `ch01.html`，体验纯原生 DOM 操作带来的繁琐流程，以及大量 DOM 更新时的性能卡顿：
...
<body>
  ...
  <div class="card">
    <p style="margin-top: 0;"><strong>性能实验：</strong>点击下方按钮，一次性插入 5000 个任务，<br>观察浏览器的卡顿。</p>
