...
![chapter illustration](../images/ch04_big_idea.png)

## 4.1 简化的渴望

Student 疲惫地靠在椅背上。上一章的 MVC 模式虽然精准，但那些错综复杂的事件监听器让他感到窒息。

**Student**：Master，为了所谓的“精准更新”，我们不得不手动编写大量的 `on('change')` 代码。每次添加一个新功能，我都要小心翼翼地去绑定事件、解绑事件。这太累了。
我怀念第二章里的那个“模板引擎”。虽然它性能差、体验差，但它真的很简单——只要数据变了，重新调用一下 `renderApp()` 也就行了。

**Master**：你察觉到了问题的本质。
在 MVC 中，我们为了性能（只更新局部 DOM），牺牲了开发的简洁性（手动管理依赖）。
在模板时代，我们拥有了开发的简洁性（全量刷新），却牺牲了性能和体验。

**Student**：难道就没有两全其美的方法吗？
我想像写模板一样写代码（声明式），但又希望它运行起来像 MVC 一样快（细粒度更新）。

换句话说，我想要一个函数——给它当前的状态，它直接告诉我界面应该长什么样。UI 就是状态的函数。

**Master**：你刚才用一句话概括了前端框架历史上最重要的思想转变。用公式来表达就是：

$$ UI = f(state) $$

2011 年，Facebook 的工程师 Jordan Walke 面对同样的困境时，做出了完全相同的思考。后来这个思想演变成了 React 框架的核心。而我们接下来要做的，就是亲手实现这个思想。

## 4.2 重新思考“刷新”

**Master**：Student，假设一个理想的世界——浏览器的 DOM 操作极其快速，快到忽略不计。在那个世界里，你会怎么写代码？

**Student**：那我就直接用 `innerHTML` 每次重绘整个页面啊！那样最简单，根本不需要什么 `Model`，也不需要监听器。

**Master**：没错。可惜，我们在第二章已经见识了全量重绘的代价——不仅性能昂贵，还会销毁整棵 DOM 树，导致输入框失去焦点、用户状态丢失。问题出在哪？

**Student**：出在直接操作真实 DOM 上。每次全量重建太浪费了。

**Master**：那如果我们不直接操作真实 DOM 呢？先退一步想——能不能在真正动手之前，先做一次“演习”？

**Student**：“演习”？您是说……先在脑子里想好要改什么，再动手？

**Master**：差不多。但不是在“脑子里”——是在 JavaScript 里。如果我们不把 UI 渲染成真实的 DOM，而是渲染成一个普通的 **JavaScript 对象**呢？

**Student**：JavaScript 对象？您是说用一个对象来描述页面的结构……比如 `{ tag: 'div', children: [...] }` 这样的？

**Master**：正是。这只是一些普通的 JS 对象，创建它们几乎不花时间。现在，假设你已经有了上一次渲染的对象树，又生成了一棵新的对象树——接下来你会做什么？

**Student**：把两棵树做对比……找出哪些地方不同！

**Master**：然后呢？

**Student**：然后只把不同的部分应用到真实的 DOM 上！这样就不用每次都重建整棵 DOM 树了！

**Master**：现在把这两步连起来——对开发者来说，体验是怎样的？

**Student**：对开发者来说，每次都重新生成了整棵对象树——代码写起来像模板一样简单。但对浏览器来说，只有真正变化的节点被更新——性能接近手动优化的 MVC。唯一的代价是多一些 CPU 计算去做对比……等等，这不就是两全其美吗？！

**Master**：你刚才一步步推导出来的，正是虚拟 DOM 的核心思想——他把这个"JavaScript 对象树"称为 **虚拟 DOM (Virtual DOM)**。

## 4.3 纯粹的映射

**Master**：既然你已经推导出了核心思想，让我们来实践它。为了聚焦在 VNode 的结构和差异对比上，我们暂时用一个更简单的 Counter 来演示，等引擎成熟后再回到 Todo List。
首先，我们要改变描述 UI 的方式。在模板时代，我们用字符串描述 UI：
`'<li>' + todo.text + '</li>'`

而现在，我们用 **数据结构** 描述 UI。

**Student**：用对象而不是字符串来描述 UI，是因为对象可以逐层对比差异，而字符串很难做到这一点吧？

**Master**：正是如此。对象是结构化的，天然适合算法分析。
...
function render(state) {
  return {
    ...
    children: [
      ...
      {
        tag: 'button',
        props: { onclick: increment }, // 注意：这是函数引用，不是字符串！
        children: ['Add']
      },
      {
        tag: 'ul',
...
**Master**：看，这个 `vdom` 对象就是当前状态下 UI 的 **快照 (Snapshot)**。
注意两个关键点：
...
**Student**：这一步很快，因为我只是创建了一些 JS 对象，没有碰真实的 DOM。

**Master**：对。接下来的魔法，就在于如何把这两个快照之间的“差异”，转化为真实 DOM 的操作指令。

> 💡 **JSX 小预告**：这个 `h()` 函数正是 JSX 编译的目标。当你在 React 中写 `<button onClick={fn}>Add</button>` 时，编译器（如 Babel）会将它转换为 `React.createElement('button', { onClick: fn }, 'Add')` —— 和我们的 `h('button', { onclick: fn }, ['Add'])` 是一回事。我们将在第六章详细介绍 JSX。

## 4.4 数据的唯一真相 (Single Source of Truth)

**Master**：在这个模型中，数据是如何流动的？
...
**Master**：但这正是天才的洞见。Jordan 意识到，只要虚拟 DOM 足够快，我们就可以为了**开发体验 (Developer Experience)** 而牺牲一点点**运行时性能 (Runtime Performance)**。

## 4.6 一切就绪

**Master**：React 并不是什么神奇的黑科技，它只是做了一个大胆的权衡：
它引入了额外的 CPU 计算（生成虚拟 DOM、对比差异）和内存开销（始终在内存中保存一份完整的虚拟 DOM 树），来换取开发者心智负担的减轻（不再手动管理 DOM 更新）。

**Student**：所以代价是 CPU 和内存，换来的是开发效率和可维护性。考虑到现代设备的性能越来越强，这笔账算下来还是划算的。

**Master**：但是，Student，光有理念是不够的。你说“对比新旧差异”，这具体该怎么做？
如果我想把这个 `vdom` 变成真实的界面，该怎么写 `mount` 函数？
如果状态变了，我怎么通过 `patch` 函数只更新变化的部分？

**Student**：这……似乎涉及到了复杂的算法。

**Master**：下一章，我们将深入引擎盖之下，亲手打造这个核心引擎。
...
将以下代码保存为 `ch04.html`。
这一章我们还没有实现 Diff 算法，但我们可以先看看 **Virtual DOM** 到底长什么样。下一章我们将用 `patch` 函数替代这里的 `innerHTML`。
...
<body>
  ...
  <p>点击按钮，观察 Virtual DOM 对象如何随状态变化。
     注意：在这一章里我们还在用 innerHTML（下一章将会用 patch 替代它）。</p>
  ...
  <script>
    ...
    // 3. 模拟渲染 
    // ⚠️ 这里仍在用 innerHTML —— 下一章的 patch 将替代它
    function updateApp() {
      ...
      // 简单粗暴地更新视图 (下一章我们将用 Diff + Patch 替代)
